...
- `.env` file populated with ERPNext API keys (use `.env.example` as a template)
...
### 3. Launch Application & Database
This will build the Java jar with the SkyWalking Agent attached and start the Postgres database.
```powershell
...
- **JDK**: 21 (Temurin)
- **Storage**: PostgreSQL 15 + BanyanDB (Observability)
- **Observability**: Apache SkyWalking 9.5.0 with Java Agent integrated in `Dockerfile`.
- **Scaling Analysis**: 
  - **100 RPS Ready**: BanyanDB is utilized to save ~8GB of RAM compared to Elasticsearch.
  - **Cache**: Dedicated Redis instance to offload ERPNext API pressure.

---

## 🧪 Testing
To run local unit and integration tests (ensure your local Postgres or Docker Postgres is accessible):
```powershell
mvn test
```
